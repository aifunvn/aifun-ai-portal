...
import { render as renderRun,           init as initRun           } from '../pages/builder-run.js';
...
async function init() {
  ...
  router.register('/run', async () => { if (!(await requireAuth())) return; const id = new URLSearchParams(window.location.hash.split('?')[1] ?? '').get('builderId') ?? ''; mountPage('/run', 'Run Builder', renderRun(id), () => initRun(id)); });
